...
use crate::{GameState, actions::{Actions, Tool, update_mouse_click}, components::{self, Deleteable, RaycastSet}};
...
fn handle_wall_building(mut actions: ResMut<Actions>, mut commands: Commands, mut preliminary_q: Query<(&mut PreliminaryWall, Entity, &mut Path, &Transform)>, mut pancam_q: Query<&mut PanCam>) {
    ...
    if preliminary_q.iter().len() == 1{
        let (mut preliminary_wall, entity, mut path, transform) = preliminary_q.single_mut();
